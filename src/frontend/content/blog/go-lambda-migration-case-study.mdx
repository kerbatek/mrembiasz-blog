...
Imagine you have a small serverless backend for your application. The backend is
implemented with AWS Lambda, and the handlers are small: they parse a request,
call one AWS service, and return a response.

After deploying the backend, you notice that some requests are much slower than
others. You check CloudWatch metrics and see that cold starts are taking a long
time.

This matters for several reasons:

- Cold starts are not free: AWS bills for the time spent in the Lambda runtime,
  even before your handler code starts doing useful work.
- Cold starts affect user experience: if a user hits a cold Lambda, they wait
  longer for the response.

In this blog app, that backend handles post view analytics. The write path
accepts a view event, sends it through SNS and SQS, and updates a DynamoDB
counter:
...
<figcaption>
Architecture: CloudFront forwards the request to API Gateway, which invokes the
validator Lambda. The validator publishes to SNS, SNS fans out to SQS, and the
aggregate worker Lambda consumes the queue and updates DynamoDB.
</figcaption>
...
I call the first request in a new log stream "cold-ish" here because it includes
the runtime setup cost visible in the Lambda report row.
...
duration, but warm duration, billed duration, and max memory used also improved.
